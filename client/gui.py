...
def get_i18n(config):
    ...
    i18n_path = os.path.join(os.path.dirname(
        base_path), "i18n", f"{lang}.json")
...
class ASRGui(ctk.CTk):
    ...
    def setup_ui(self):
        ...
        self.logo_label = ctk.CTkLabel(
            self.sidebar_frame, text="WTAKO ASR IME", font=ctk.CTkFont(size=20, weight="bold"))
        self.logo_label.grid(row=0, column=0, padx=20, pady=(20, 10))

        self.start_button = ctk.CTkButton(self.sidebar_frame, text=self.i18n.get(
            "start_asr", "Start ASR"), command=self.toggle_client)
        self.start_button.grid(row=1, column=0, padx=20, pady=10)

        self.appearance_mode_label = ctk.CTkLabel(self.sidebar_frame, text=self.i18n.get(
            "appearance_mode", "Appearance Mode:"), anchor="w")
        ...
        self.language_label = ctk.CTkLabel(
            self.sidebar_frame, text=self.i18n.get("language", "Language:"), anchor="w")
        ...
        self.github_label = ctk.CTkLabel(self.sidebar_frame, text="GitHub", font=ctk.CTkFont(
            size=12, underline=True), cursor="hand2")
        self.github_label.grid(row=9, column=0, padx=20, pady=(10, 10))
        self.github_label.bind(
            "<Button-1>", lambda e: webbrowser.open("https://github.com/Saren-Arterius/WTAKO-ASR-IME"))
        ...
        self.status_label = ctk.CTkLabel(self.status_frame, text=self.i18n.get(
            "status_disconnected", "Status: Disconnected"), font=ctk.CTkFont(size=16))
        self.status_label.grid(row=0, column=0, sticky="w")

        self.clear_button = ctk.CTkButton(self.status_frame, text=self.i18n.get(
            "clear_log", "Clear Log"), width=80, command=self.clear_log)
        ...
        self.textbox.insert("0.0", self.i18n.get(
            "transcription_placeholder", "Transcriptions will appear here...\n"))
        ...
        self.use_custom_url = ctk.BooleanVar(
            value=not self.config.get("use_local_server", True))
        self.url_checkbox = ctk.CTkCheckBox(self.settings_frame, text=self.i18n.get(
            "asr_server_url", "ASR Server URL"), variable=self.use_custom_url, command=self.toggle_url_entry)
        ...
        if not self.use_custom_url.get():
            self.server_entry.insert(0, self.i18n.get(
                "local_server_placeholder", "Will start a local server automatically"))
            self.server_entry.configure(state="disabled", text_color="grey")
        else:
            self.server_entry.insert(0, self.config.get(
        ...
        ctk.CTkLabel(self.settings_frame, text=self.i18n.get(
            "input_device", "Input Device:")).grid(row=1, column=0, padx=10, pady=5, sticky="w")
        devices = sd.query_devices()
        device_names = [f"{i}: {d['name']}" for i, d in enumerate(
            devices) if d['max_input_channels'] > 0]
        self.device_option = ctk.CTkOptionMenu(
            self.settings_frame, values=device_names, command=self.on_device_change)
        self.device_option.grid(row=1, column=1, padx=10, pady=5, sticky="ew")

        self.volume_label = ctk.CTkLabel(
            self.settings_frame, text=self.i18n.get("volume", "Volume:"))
        ...
        ctk.CTkLabel(self.settings_frame, text=self.i18n.get("hotkey", "Hotkey:")).grid(
        ...
        common_keys = ["f12", "f1", "f2", "f3", "f4", "f5", "f6", "f7", "f8", "f9",
                       "f10", "f11", "caps lock", "insert", "home", "page up", "page down", "end"]
        ...
        self.hotkey_option = ctk.CTkOptionMenu(
            self.hotkey_frame, values=common_keys)
        self.hotkey_option.grid(
            row=0, column=0, padx=(0, 5), pady=0, sticky="ew")
        self.hotkey_option.set(current_hotkey)

        self.record_button = ctk.CTkButton(self.hotkey_frame, text=self.i18n.get(
            "record", "Record"), width=60, command=self.start_recording_hotkey)
        ...
        self.system_prompt_label = ctk.CTkLabel(
            self.settings_frame, text=self.i18n.get("system_prompt", "System Prompt:"))
        ...
        self.system_prompt_entry.insert(
            0, self.config.get("glm", {}).get("system_prompt", ""))

        # SenseVoice specific settings
        self.sensevoice_frame = ctk.CTkFrame(
            self.settings_frame, fg_color="transparent")
        self.sensevoice_frame.grid(row=5, column=0, columnspan=2, sticky="ew")
        self.sensevoice_frame.grid_columnconfigure(1, weight=1)

        ctk.CTkLabel(self.sensevoice_frame, text=self.i18n.get(
            "num_threads", "Threads:")).grid(row=0, column=0, padx=10, pady=5, sticky="w")
        self.threads_entry = ctk.CTkEntry(self.sensevoice_frame)
        self.threads_entry.grid(row=0, column=1, padx=10, pady=5, sticky="ew")
        self.threads_entry.insert(
            0, str(self.config.get("sensevoice", {}).get("num_threads", 2)))

        ctk.CTkLabel(self.sensevoice_frame, text=self.i18n.get("sensevoice_language",
                     "SenseVoice Language:")).grid(row=1, column=0, padx=10, pady=5, sticky="w")
        self.sv_lang_option = ctk.CTkOptionMenu(self.sensevoice_frame, values=[
                                                "auto", "zh", "en", "ja", "ko", "yue"])
        self.sv_lang_option.grid(row=1, column=1, padx=10, pady=5, sticky="ew")
        self.sv_lang_option.set(self.config.get(
            "sensevoice", {}).get("language", "auto"))

        ctk.CTkLabel(self.sensevoice_frame, text=self.i18n.get(
            "provider", "Provider:")).grid(row=2, column=0, padx=10, pady=5, sticky="w")
        self.provider_option = ctk.CTkOptionMenu(
            self.sensevoice_frame, values=["cpu", "cuda", "coreml"])
        self.provider_option.grid(
            row=2, column=1, padx=10, pady=5, sticky="ew")
        self.provider_option.set(self.config.get(
            "sensevoice", {}).get("provider", "cpu"))

        # Whisper specific settings
        self.whisper_frame = ctk.CTkFrame(
            self.settings_frame, fg_color="transparent")
        self.whisper_frame.grid(row=5, column=0, columnspan=2, sticky="ew")
        self.whisper_frame.grid_columnconfigure(1, weight=1)

        ctk.CTkLabel(self.whisper_frame, text=self.i18n.get(
            "whisper_device", "Device:")).grid(row=0, column=0, padx=10, pady=5, sticky="w")
        self.whisper_device_option = ctk.CTkOptionMenu(
            self.whisper_frame, values=["cuda", "cpu"])
        self.whisper_device_option.grid(
            row=0, column=1, padx=10, pady=5, sticky="ew")
        self.whisper_device_option.set(
            self.config.get("whisper", {}).get("device", "cuda"))

        ctk.CTkLabel(self.whisper_frame, text=self.i18n.get(
            "whisper_language", "Language:")).grid(row=1, column=0, padx=10, pady=5, sticky="w")
        self.whisper_lang_option = ctk.CTkOptionMenu(
            self.whisper_frame, values=["auto", "en", "zh", "ja", "ko", "yue"])
        self.whisper_lang_option.grid(
            row=1, column=1, padx=10, pady=5, sticky="ew")
        self.whisper_lang_option.set(self.config.get(
            "whisper", {}).get("language", "auto"))

        ctk.CTkLabel(self.whisper_frame, text=self.i18n.get(
            "whisper_task", "Task:")).grid(row=2, column=0, padx=10, pady=5, sticky="w")
        self.whisper_task_option = ctk.CTkOptionMenu(
            self.whisper_frame, values=["transcribe", "translate"])
        self.whisper_task_option.grid(
            row=2, column=1, padx=10, pady=5, sticky="ew")
        self.whisper_task_option.set(self.config.get(
            "whisper", {}).get("task", "transcribe"))

        # Qwen specific settings
        self.qwen_frame = ctk.CTkFrame(
            self.settings_frame, fg_color="transparent")
        ...
        qwen_supported_langs = ["auto", "Chinese", "English", "Cantonese", "Arabic", "German", "French", "Spanish", "Portuguese", "Indonesian", "Italian", "Korean", "Russian", "Thai",
                                "Vietnamese", "Japanese", "Turkish", "Hindi", "Malay", "Dutch", "Swedish", "Danish", "Finnish", "Polish", "Czech", "Filipino", "Persian", "Greek", "Romanian", "Hungarian", "Macedonian"]
        self.qwen_lang_option = ctk.CTkOptionMenu(
            self.qwen_frame, values=qwen_supported_langs)
        self.qwen_lang_option.grid(
            row=1, column=1, padx=10, pady=5, sticky="ew")
        self.qwen_lang_option.set(self.config.get(
            "qwen_asr", {}).get("language", "auto"))

        ctk.CTkLabel(self.qwen_frame, text=self.i18n.get("qwen_model", "Model:")).grid(
            row=2, column=0, padx=10, pady=5, sticky="w")
        self.qwen_model_option = ctk.CTkOptionMenu(
            self.qwen_frame, values=["Qwen/Qwen3-ASR-1.7B", "Qwen/Qwen3-ASR-0.6B"])
        self.qwen_model_option.grid(
            row=2, column=1, padx=10, pady=5, sticky="ew")
        ...
        self.disable_log_checkbox = ctk.CTkCheckBox(self.settings_frame, text=self.i18n.get(
            "disable_log", "Disable Transcribe Log"), variable=self.disable_log_var)
        self.disable_log_checkbox.grid(
            row=6, column=0, padx=10, pady=5, sticky="w")

        self.opencc_frame = ctk.CTkFrame(
            self.settings_frame, fg_color="transparent")
        self.opencc_frame.grid(row=6, column=1, padx=10, pady=5, sticky="ew")
        self.opencc_frame.grid_columnconfigure(1, weight=1)

        ctk.CTkLabel(self.opencc_frame, text=self.i18n.get("opencc_convert", "OpenCC Convert:")).grid(
        ...
        self.opencc_option = ctk.CTkOptionMenu(
            self.opencc_frame, values=list(self.opencc_map.keys()))
        ...
        self.save_button = ctk.CTkButton(self.settings_frame, text=self.i18n.get(
            "save_config", "Save Config"), command=self.save_config_event)
    ...
    def transition_to(self, new_state, error_msg=None):
        ...
        def update_ui():
            ...
            self.device_option.configure(
                state="disabled" if is_running else "normal")
            self.hotkey_option.configure(
                state="disabled" if is_running else "normal")
            self.record_button.configure(
                state="disabled" if is_running else "normal")
            self.backend_option.configure(
                state="disabled" if is_running else "normal")
            ...
            if new_state == AppState.DISCONNECTED:
                self.update_status(self.i18n.get("status_disconnected", "Disconnected").replace(
                    self.i18n.get("status_prefix", "Status: "), ""))
                self.start_button.configure(text=self.i18n.get(
                    "start_asr", "Start ASR"), fg_color=ctk.ThemeManager.theme["CTkButton"]["fg_color"])
            elif new_state == AppState.STARTING:
                self.update_status(self.i18n.get(
                    "status_starting", "Starting..."), "orange")
                self.start_button.configure(text=self.i18n.get(
                    "stop_client", "Stop Client"), fg_color="red")
            elif new_state == AppState.WAITING_FOR_SERVER:
                self.update_status(self.i18n.get(
                    "status_waiting", "Waiting for ASR Server..."), "orange")
            elif new_state == AppState.LISTENING:
                self.update_status(self.i18n.get(
                    "status_listening", "Listening"), "green")
                ...
                self.update_status(self.i18n.get("status_error", "Error: {error_msg}").format(
                    error_msg=error_msg or 'Unknown error'), "red")
                self.start_button.configure(text=self.i18n.get(
                    "start_asr", "Start ASR"), fg_color=ctk.ThemeManager.theme["CTkButton"]["fg_color"])
    ...
    def start_volume_monitor(self):
        ...
        threading.Thread(
            target=self._start_volume_monitor_thread, daemon=True).start()
    ...
    def stop_volume_monitor(self):
        ...
        threading.Thread(
            target=self._stop_volume_monitor_thread, daemon=True).start()
    ...
    def _start_volume_monitor_thread(self):
        ...
        try:
            ...
            try:
                ...
                self.volume_stream = sd.InputStream(
                    device=device_id, channels=1, callback=audio_callback, blocksize=1024)
    ...
    def update_ui_texts(self):
        self.title(self.i18n.get("title", "WTAKO ASR IME"))
        self.start_button.configure(text=self.i18n.get(
            "start_asr", "Start ASR") if not self.is_running else self.i18n.get("stop_client", "Stop Client"))
        self.appearance_mode_label.configure(
            text=self.i18n.get("appearance_mode", "Appearance Mode:"))
        self.language_label.configure(
            text=self.i18n.get("language", "Language:"))
        self.clear_button.configure(
            text=self.i18n.get("clear_log", "Clear Log"))
        self.url_checkbox.configure(text=self.i18n.get(
            "asr_server_url", "ASR Server URL"))
        ...
        for widget in self.settings_frame.winfo_children():
            if isinstance(widget, ctk.CTkLabel):
                ...
                # Fallback check
                if text == "Input Device:" or text == self.i18n.get("input_device", "Input Device:"):
                    widget.configure(text=self.i18n.get(
        ...
        self.disable_log_checkbox.configure(
            text=self.i18n.get("disable_log", "Disable Transcribe Log"))
        ...
        for widget in self.opencc_frame.winfo_children():
            if isinstance(widget, ctk.CTkLabel):
                widget.configure(text=self.i18n.get(
                    "opencc_convert", "OpenCC Convert:"))

        self.save_button.configure(
            text=self.i18n.get("save_config", "Save Config"))
    ...
    def toggle_url_entry(self):
        if self.use_custom_url.get():
            self.server_entry.configure(
                state="normal", text_color=ctk.ThemeManager.theme["CTkEntry"]["text_color"])
    ...
    def on_key_pressed(self, event):
        ...
        self.record_button.configure(text=self.i18n.get(
            "record", "Record"), fg_color=ctk.ThemeManager.theme["CTkButton"]["fg_color"])
    ...
    def start_client(self):
        ...
        device_str = self.device_option.get()
        if not device_str or ":" not in device_str:
            self.transition_to(AppState.ERROR, self.i18n.get(
        ...
        self.client_thread = threading.Thread(
            target=self.run_client, args=(device_id,), daemon=True)
    ...
    def run_client(self, device_id):
        try:
            ...
            try:
                ...
                self.client.input_sample_rate = int(
                    dev_info['default_samplerate'])
                self.client.input_channels = dev_info['max_input_channels']
            except Exception as e:
                self.transition_to(AppState.ERROR, self.i18n.get(
                    "invalid_device_error", "Invalid device - {e}").format(e=e))
            ...
            # Start threads
            threading.Thread(
                target=self.client.socket_listener, daemon=True).start()
            threading.Thread(target=self.client.recording_loop,
                             daemon=True).start()
            ...
            while self.is_running and self.client and not self.client.stop_event.is_set():
                ...
                is_active = self.client.is_recording_dict.get(
                    "active") or self.client.is_recording_dict.get("internal_active")
                ...
                if is_active:
                    ...
                    try:
                        ...
                        self.transition_to(AppState.ERROR, self.i18n.get(
                            "audio_error", "Audio Error: {e}").format(e=e))
                        break
                else:
